...
- The IP rate limit in `generate-kitbash` (via `checkRateLimit`) is split by payment status rather than shared: free generations keep the existing 5/hour, 20/day ceiling, while paid rerolls get their own counter keys (`reroll:hour:` / `reroll:day:`) at 20/hour, 100/day. The limit exists to bound worst-case Gemini spend, which a user burning 60,000 GNRM per generation isn't threatening the way a free-rider is — under one shared budget a user who minted once and rerolled four times was rejected by the same counter as an abuser, *after* already paying.
